...
<script setup lang="ts">
	...
	function onDeleteTask(taskToDelete: { id: string; task: string; done: boolean }) {
		axios.delete(
			`https://vue-todo-57da6-default-rtdb.firebaseio.com/todos/${taskToDelete.id}.json`
		);
	...
	function onEditTask(taskToEdit: { id: string; task: string; done: boolean }) {
		axios
			.put(`https://vue-todo-57da6-default-rtdb.firebaseio.com/todos/${taskToEdit.id}.json`, {
	...
	function onDoneTask(taskToDone: { id: string; task: string; done: boolean }) {
		axios
			.put(`https://vue-todo-57da6-default-rtdb.firebaseio.com/todos/${taskToDone.id}.json`, {
	...
	function onSearchHandler(value: string) {
		const arr = [...allTasks.value];
		const filtered = arr.filter((item) => item.task.toLowerCase().includes(value.toLowerCase()));
